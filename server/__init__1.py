...
# from server.get_BondData import get_transaction_data, get_mktPrice_data
...
@app.route("/api/getAllData/<string:date>", methods=['POST', 'GET'])
def get_AllData(date):
    try:
        # Mock data loading
        mktPrice, transaction, instn_base_info, df_chain_data, instn_dict = load_data()

        # Store the data in Flask's context (per-request)
        g.transaction_data = transaction
        g.mktPrice_data = mktPrice
        g.transactionTsne_data = transaction
        g.volume_data = transaction
        g.instn_dict = instn_dict
        ...
        # Return response
        return jsonify({'bondSummaryData': bondSummaryData})

    except Exception as e:
        return jsonify({'error': str(e)}), 500

@app.route("/api/getAllData1/<string:date>", methods=['POST','GET'])
def get_AllData1(date):
    # transaction = get_transaction_data(date)
    # mktPrice = get_mktPrice_data(date)
    # transactionTsne = transaction
    # volume = transaction
    
    # Store the data in Flask's context
    # g.transaction_data = transaction
    # g.mktPrice_data = mktPrice
    # g.transactionTsne_data = transactionTsne
    # g.volume_data = volume
    # Mock data
    print('Mock data')
    mktPrice, transaction, instn_base_info, df_chain_data, instn_dict = load_data()
    g.transaction_data = transaction
    ...
    return jsonify({'mktPrice': mktPrice, 'transaction': transaction, 'transactionTsne': transaction, 'volume': transaction})

    # Return mock data as JSON
    df_MarketPrice, df_transaction, instn_base_info, df_chain_data, instn_dict = load_data()
    json_transaction, json_mkt = get_details_data(df_MarketPrice, df_transaction)
    return jsonify({'mktPrice': mktPrice, 'transaction': transaction, 'transactionTsne': transactionTsne, 'volume': volume})
...
def getTheBondData_transaction(bond_cd):
    ...
    # 2. Transaction data
    # Filter the data for 'date'
    # print('df_transaction', bond_cd)
    # print('df_transaction', df_transaction)
    # df_transaction = g.transaction_data
    df_transaction['date'] = df_transaction['txn_dt']
    filtered_json_transaction = df_transaction[df_transaction['date'].isin(workday_list)]
...
def getTheBond_MarketPrice(bond_cd):
    ...
    # 1. MarketPrice data
    # Filter the data for 'date'
    df_MarketPrice = df_MarketPrice
    print('df_MarketPrice:', df_MarketPrice)
    df_MarketPrice['date'] = df_MarketPrice['mkt_data_upd_tm'].str[:10]
    filtered_json_mkt = df_MarketPrice[df_MarketPrice['date'].isin(workday_list)]
...
def get_institution_types():
    json_data = json.dumps(g.instn_dict, ensure_ascii=False)
