...
# def get_broker_data(date):
#     # 将参数 `date` 动态地插入到 SQL 查询中
#     sql = f"""
#     select * from dpa.v_cbt_mny_brkrg_rltm_dl_non_cnex
#     where txn_dt = %s
#     limit 30
#     """
#     # 使用参数化查询执行 SQL
#     df_trade = pg_select(sql, (date,))
#     broker_data = df_trade#[columns_to_export]
#     return broker_data
...
def get_valuation_data(date):
    ...
    # 使用参数化查询执行 SQL
    df_trade = pg_select(sql, (date,))
    columns_to_export = ["bond_cd", 'bnds_nm', 'vltn_dt', 'vltn_net_prc', 'yld_to_mrty', 'bond_tp']
    return df_trade[columns_to_export]

# # 将参数 `date` 动态地插入到 SQL 查询中
# sql = f"""
# select *
# from dpa.cntrl_dpstry_vltn
# where vltn_dt = '2024-10-09'
# limit 100000
# """
# # 使用参数化查询执行 SQL
...
def get_broker_data(date):
    ...
    # 构建 SQL 查询，使用 >= 和 <= 来匹配当天的时间范围
    sql = f"""
    select * from dpa.v_cbt_mny_brkrg_rltm_dl_non_cnex
    where dl_tm >= %s AND dl_tm <= %s
    limit 30000
    """
    # 使用参数化查询执行 SQL，传递开始时间和结束时间
    df_trade = pg_select(sql, (date_start, date_end))
    
    # 假设 `columns_to_export` 已定义，选择需要的列
    broker_data = df_trade#[columns_to_export]
    return broker_data
