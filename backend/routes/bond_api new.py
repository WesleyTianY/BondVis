...
def load_data_data_center(query_date, bond_id):
    ...
    # 筛选数据
    broker_data_filtered = broker_data[(broker_data['bond_cd'] == int(bond_id)) & 
                                       (pd.to_datetime(broker_data['dl_tm']).dt.date == selected_date.date())].copy()
    ...
    return broker_data_filtered, ndm_transaction_filtered, valuation_filtered
...
def data():
    ...
    # 获取债券名称
    bnds_nm = ndm_transaction_sorted_data["bnds_nm"].unique()
