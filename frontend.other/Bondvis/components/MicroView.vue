...
export default {
  ...
  methods: {
    ...
    async drawMultiCharts(data, bond_cd, endDay, duration_days, theLimits, colorMapping) {
      ...
      class MultiChart {
        ...
        lassoFunction() {
          ...
          function highNewEditedCircles(Container) {
            ...
            if (getChangedTransaction == []){ return }
          ...
          var lasso_end = function() {
            ...
            // lqy: 下面这里触发了tempSelection(state, payload)方法，导致state.globalLassoData[payload.bondcd]被更新然后被watch到
            store.dispatch('lassoInteraction/updateTempSelection', payload_temp)
            // console.log("#MicroView/lassoFunction: payload_temp", payload_temp)
      ...
      // console.log('drawMultiCharts called1')
    ...
    async fetchData() {
      ...
      try {
        // 直接创建 MultiData 实例并调用 generateData 方法
        const multiDataInstance = new MultiData(this.bond_cd, this.end_date, this.duration_days);
        await multiDataInstance.generateData(this.bond_cd, this.end_date, this.duration_days);
        ...
        // 调用绘图方法
        // console.log('drawMultiCharts, 849')
