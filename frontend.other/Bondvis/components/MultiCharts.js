...
class MultiChart {
  constructor(data, bond_cd, endDay, duration_days, theLimits, colorMapping, vueComponent) {
    if (!Array.isArray(theLimits) || theLimits.length < 2) {
      console.log('theLimits', data, bond_cd)
      throw new Error('theLimits should be an array with at least two elements');
    }
    ...
    var InstitutionContainer = this.chartContainer.append('svg')
      .attr('width', this.width)
      .attr('height', this.height)
      .attr('name', 'InstitutionContainer')
    this.InstitutionContainer = InstitutionContainer
  ...
  async drawMultiDayData(dimension) {
    ...
    // console.log('singleFeaList sortedSingleFeaList', sortedSingleFeaList, this.allData, dimension)
    // 这里有问题
    const groupedData = {}
  ...
  drawTrasactionScatter(data, xOffset, time_Period, startTimePoint, endTimePoint, groupCount) {
    ...
    this.lassoContainer.append('g')
      ...
      .attr('r', d => Math.sqrt(d.transactionVolume*50) / 3.14)
      // .attr('r', d => rScale(Math.sqrt(d.transactionVolume / 3)))
      .attr('fill', d => {
  ...
  drawInstitution(data, xOffset, time_Period, startTimePoint, endTimePoint, groupCount) {
    ...
    this.lassoContainer.append('g')
      ...
      .attr('cx', d => xScale(d.timeStamp))
      .attr('cy', d => yScale(d.netPrice))
      .attr('r', d => rScale(Math.sqrt(d.transactionVolume) / 3.14))
      .attr('fill', d => {
          // console.log('d.byr_instn_tp', this.colorMapping)
          const colorMapping = this.colorMapping[d.byr_instn_tp];
  ...
  lassoFunction(lassoContainer, vueComponent, store) {
...
// 定义与 lasso 相关的函数
function highNewEditedCircles(lassoContainer, store) {
  const getChangedTransaction = store.getters['lassoInteraction/getChangedTransaction'];
  if (getChangedTransaction.length === 0) {
    return;
  }
  ...
  for (let i = 1; i < AllNodesList.length - 1; i++) {
    ...
    const isSelected = getChangedTransaction.some(item => item.transactionId === nodeId);
    if (isSelected) {
      d3.select(thisNode).style('stroke', color_selected).style('stroke-width', 6);
    }
  }
}

function highlightCircles(lassoContainer, store) {
  const globalLassoData = store.getters['lassoInteraction/getGlobalLassoData'];
  if (globalLassoData == undefined) {
    return;
  }
  ...
  for (let i = 1; i < AllNodesList.length - 1; i++) {
    ...
    const isSelected = globalLassoData.some(item => item.transactionId === nodeId);
    if (isSelected) {
      d3.select(node).style('stroke', color_selected).style('stroke-width', 2);
    } else {
      d3.select(node).style('stroke', color_default).style('stroke-width', 1);
    }
  }
}

function setupLasso(lassoContainer, vueComponent, bond_cd, store) {
  const lasso_start = function () {
    console.log('start');
    lassoInstance.items().classed('not_possible', true).classed('selected', false);
  };

  const lasso_draw = function () {
    console.log('draw');
    lassoInstance.possibleItems().classed('not_possible', false).classed('possible', true);
    lassoInstance.notPossibleItems().classed('not_possible', true).classed('possible', false);
  };

  const lasso_end = function () {
    console.log('end');
    lassoInstance.items().classed('not_possible', false).classed('possible', false);
    lassoInstance.selectedItems().classed('selected', true);
    const selectedElements = lassoInstance.selectedItems();
    const selectedData_temp = [];
    selectedElements.each(function (d) {
      const circleData = d3.select(this).data()[0];
    ...
    store.dispatch('lassoInteraction/updateTempSelection', payload_temp);
    highNewEditedCircles(lassoContainer, store);
    highlightCircles(lassoContainer, store);
  };
...
export async function drawMultiCharts(data, bond_cd, endDay, duration_days, theLimits, colorMapping, lassoContainer, vueComponent, store) {
  const drawMultiChartsInstance = new MultiChart(data, bond_cd, endDay, duration_days, theLimits, colorMapping);

  try {
    await drawMultiChartsInstance.drawMultiDayData('mktPrice');
    await drawMultiChartsInstance.drawMultiDayData('transaction');
    await drawMultiChartsInstance.drawMultiDayData('volume');
    console.log(lassoContainer, vueComponent, bond_cd, store)
    drawMultiChartsInstance.lassoFunction(lassoContainer, vueComponent, store);
    setupLasso(lassoContainer, vueComponent, bond_cd, store)
    // 返回数据包或其他需要的返回值
    return data;
  } catch (error) {
