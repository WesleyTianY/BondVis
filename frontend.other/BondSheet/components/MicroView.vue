  ...
  methods: {
    ...
    async fetchData() {
      ...
      try {
        ...
        await multiDataInstance.generateData_tsne(this.bond_cd, this.end_date, this.duration_days);
        const dataPackage = multiDataInstance.dataPackage;

        // average and dev
        const scatter_data = dataPackage.transactionTsne.map(item => item.netPrice);
