...
export const Treemap = (selection, props) => {
  ...
  //invoking treemap
  const treemapLayout = treemap().size([innerWidth, innerHeight]);
  ...
  // .attr("viewBox", `0 0 ${width} ${height}`)
    ...
    .data([null])
    .join("g")
    .attr("class", "svg__tree")
    .attr("transform", `translate(${margin.left}, ${margin.top})`)
    
    
  const grandparent = svg
    .selectAll(".grandparent")
    .data([null])
    .join("g")
  ...
  function showTooTip(d) {
    ...
    tooltip
      ...
      .style(
        "top",
        `${
          event.pageY < innerHeight / 1.25 ? event.pageY : event.pageY - 100
        }px`
      );
  }
};
