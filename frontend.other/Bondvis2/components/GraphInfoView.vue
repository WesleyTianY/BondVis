<template>
  <div>
    <b-card no-body>
      ...
      <b-card-body class="graph_body" style="height: 680px; padding: 5px 0px 5px 0px">
        <svg :id="'globalGraphView'" style="height: 680px; width: 900px;" />
      </b-card-body>
    </b-card>
  </div>
</template>
  ...
  methods: {
    ...
    plotGraph1() {
      fetch('../datasets/miserables.json').then(res => res.json()).then(data => {
        // Assign a random size to each node
        data.nodes.forEach(node => {
          node.size = Math.random() * 10 + 5; // Random size between 5 and 15
        });
        ...
        function nodePaint({ id, x, y, size }, color, ctx) {
          ctx.fillStyle = color;
          ctx.beginPath();
