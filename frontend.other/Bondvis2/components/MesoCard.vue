  ...
  <div>
    <b-card no-body style="border: 0px">
      <b-card-header class="small-header" style="font-size:12px; height:25px;">
        ...
        <el-switch v-model="tagsView" class="drawer-switch" @change="toggleView" style="font-size: 12px; float: left; margin-right:10px; padding: 0px 5px; height: 15px; " />
        <vue-slider
          ...
          :included="true"
          style="margin-left: 20px; height: 380px; margin-top: 0px"
        ></vue-slider>
      </b-card-header>
      <b-card-body class="meso_card_body" style="height: 450px; padding: 5px 0px 5px 0px">
        <MicroViewTime v-if="showView === 'timeView'" ref="microViewRef" :bond_cd="bond_cd" :end_date="end_date" :duration_days="duration_days" :scaler_index="scaler_index" :scaler_index_high="scaler_index_high" :scaler_index_low="scaler_index_low" />
        <MicroView v-if="showView === 'freqView'" ref="microViewRef" :bond_cd="bond_cd" :end_date="end_date" :duration_days="duration_days" :scaler_index="scaler_index" :scaler_index_high="scaler_index_high" :scaler_index_low="scaler_index_low" />
      </b-card-body>
    </b-card>
  </div>
</template>
...
import MicroViewTime from './MicroView_time'
