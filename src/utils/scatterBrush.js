 ...
 * @param {d3.ScaleLinear} newX - 当前的 x 轴比例尺
 * @param {d3.ScaleLinear} newY - 当前的 y 轴比例尺
 */
export function addScatterBrushInteractions(dots, newY, newX, onBrushEnd) {
  // 获取SVG容器
  ...
  function isInBrushExtent(d, brushExtent) {
    // 将屏幕坐标转换为数据空间的坐标
    const cx = newX(d.timeStamp);  // 根据 newX 转换时间戳为 x 坐标
    const cy = newY(d.yValue);  // 根据 newY 转换 netPrice 或 yld_to_mrty 为 y 坐标
    return brushExtent &&
      cx >= brushExtent[0][0]-35 && cx <= brushExtent[1][0]-35 &&
      cy >= brushExtent[0][1]-15 && cy <= brushExtent[1][1]-15;
  }
