...
export function drawChart(containerRef, chartData, displayType, displayNodeTypes, interaction, Threshold, Threshold_trade, Threshold_BP, Threshold_trade_BP, yAxisType) {
  const Config = {
    margin: { top: 10, right: 50, bottom: 80, left: 40 },
    width: 1300,
    height: 400,
  ...
  const sortedY = yValues.slice().sort(d3.ascending);
  const medianY = d3.mean(sortedY);
  // 计算最大偏差
  const maxDeviation = d3.max(yValues, y => Math.abs(y - medianY))*1.3;

  // 基于中值和偏差计算范围
  const minY = medianY - maxDeviation;
  const maxY = medianY + maxDeviation;
  ...
  // 提取阈值逻辑
  const isNetPrice = yAxisType === 'netPrice';
  const Threshold_value = isNetPrice ? Threshold : Threshold_BP;
  const Threshold_trade_value = isNetPrice ? Threshold_trade : Threshold_trade_BP;


  // 定义放缩行为
  const zoom = d3.zoom()
    .scaleExtent([0.5, 15]) // 设置放缩范围
  ...
  function zoomed() {
    ...
    // 重新绘制其他图形
    drawValuationLineChart(valuationLineGroup.node(), valuationData, Config, yAxisType, Threshold_value, newY, newX);
    if (yAxisType !== 'netPrice') {
      drawLineChart(lineGroup.node(), brokerData, Config, Threshold_trade_value, newY, newX);
    }
    drawScatterPlot(scatterGroup.node(), transactionData, Config, yAxisType, displayNodeTypes, interaction, newY, newX);
  }

  // 绘制散点图，传入统一的 x 比例尺
  drawValuationLineChart(valuationLineGroup.node(), valuationData, Config, yAxisType, Threshold_value, y, x);
  // 绘制折线图，传入统一的 x 比例尺
  if (yAxisType !== 'netPrice') {
    drawLineChart(lineGroup.node(), brokerData, Config, Threshold_trade_value, y, x);
  }
