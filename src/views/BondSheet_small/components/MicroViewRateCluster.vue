  ...
  methods: {
    ...
    async drawMultiCharts(data, bond_cd, endDay, duration_days, theLimits, colorMapping) {
      ...
      class MultiChart {
        ...
        async drawMultiDayData(dimension) {
          ...
          // console.log('singleFeaList', singleFeaList)
          ...
          Object.entries(groupedData_final).forEach(([theDate, singleDayDataList], index) => {
            ...
            this.drawDayData(singleDayDataList, dimension, theDate, 0, 'PM', groupCount)
          })
        }
        drawDayData(data, dimension, date, xOffset, time_Period, groupCount) {
          ...
          switch (dimension) {
            ...
            case 'transaction':
              ...
              break
            case 'transactionTsne':
              this.drawTrasactionTsneScatter(data, xOffset, time_Period, startTimePoint, endTimePoint, groupCount)
              break
            // case 'volume':
            //   this.drawVolumeBar(data, xOffset, time_Period, startTimePoint, endTimePoint, groupCount)
            //   break
        ...
        drawMktPriceLine(data, xOffset, time_Period, startTimePoint, endTimePoint, groupCount) {
          ...
          defs.append('path')
            ...
            // .attr('fill', colors[4])
          const fixedYValue = this.allData.valuation[0].yld_to_mrty; // 假设你想要的固定纵轴值为 50，可以根据实际情况修改
          const line_val = d3.line()
          .x(d => xScale(d.timeStamp))
          .y(() => yScale(fixedYValue));

          const area_val = d3.area()
            .x(d => xScale(d.timeStamp))
            .y0(d => yScale(fixedYValue-0.02))  // Lower bound
            .y1(d => yScale(fixedYValue+0.02));  // Upper bound
        ...
        drawMktPriceLine1(data, xOffset, time_Period, startTimePoint, endTimePoint, groupCount) {
          // var parseTime = d3.timeParse('%Y/%m/%d %H:%M:%S')
          data.forEach(function(d) { 
        ...
        drawTrasactionTsneScatter(data, xOffset, time_Period, startTimePoint, endTimePoint, groupCount) {
          // 定义时间解析器
          data.forEach(function(d) { 
            const timeWithoutTimezone = d.timeStamp.slice(0, -6)
            d.timeStamp = timeWithoutTimezone
          ...
          const xScale = d3.scaleLinear().range([this.theWidth*0.1, (this.theWidth)])
          ...
          const yScale = d3.scaleLinear()
            // .domain([100, 103]).range([maxY, minY])
          ...
          this.lassoContainer.append('g')
            ...
            .attr('cy', d => yScale(d.yld_to_mrty))
          ...
          this.lassoContainer.append("g")
            ...
            .call(d3.axisBottom(xScale).ticks(this.width / 50))
          ...
          const fixedYValue = this.allData.valuation[0].yld_to_mrty; // 假设你想要的固定纵轴值为 50，可以根据实际情况修改
          console.log("fixedYValue", this.allData.valuation[0].yld_to_mrty)
          const line_val = d3.line()
          .x(d => xScale(d.timeStamp))
          .y(() => yScale(fixedYValue));
          ...
          const area_val = d3.area()
            ...
            .y0(d => yScale(fixedYValue-0.02))  // Lower bound
            .y1(d => yScale(fixedYValue+0.02));  // Upper bound
          this.lassoContainer.append('g').append('path')
            .datum(data)
            .attr('class', 'line')
            .attr('d', line_val)
            // .attr('fill', colors[0])
        ...
        drawVolumeBar(data, xOffset, time_Period, startTimePoint, endTimePoint, groupCount) {
          ...
          const groupedDataArray = this.bucketDataByXpos(data, bucket_num)
          console.log('groupedDataArray770:', groupedDataArray)
          // 定义时间解析器
          startTimePoint = this.formatDateToCustomDateString(startTimePoint)
          endTimePoint = this.formatDateToCustomDateString(endTimePoint)
          // this.width = 1000
        ...
        bucketDataByXpos(data, bucket_num) {
          // Calculate Xmax and Xmin
          const Xmax = Math.max(...data.map(d => d.x));
          const Xmin = Math.min(...data.map(d => d.x));
          const Granularity = (Xmax - Xmin) / bucket_num
          const groupedData = {};
          
          // Calculate the number of buckets needed
          const numberOfBuckets = Math.ceil((Xmax - Xmin) / Granularity);
          
          data.forEach(function(d) {
            // Calculate the bucket key based on the x value of the data point
            const bucketIndex = Math.floor((d.x_pos - Xmin) / Granularity);
            
            // Ensure the bucket key is within the valid range
            const bucketKey = Math.max(0, Math.min(bucketIndex, numberOfBuckets - 1)).toString();
        ...
        lassoFunction() {
          ...
          var lasso_end = function() {
            ...
            // if (payload_temp.theSelection.length > 0) {
            //   store.dispatch('lassoInteraction/updateTempSelection', payload_temp)
            // }
      ...
      await multiChartInstance.drawMultiDayData('transactionTsne')
    ...
    async fetchData() {
      ...
      try {
        ...
        await multiDataInstance.generateData_Yld_to_mrtytsne(this.bond_cd, this.end_date, this.duration_days);
        const dataPackage = multiDataInstance.dataPackage;
        console.log('dataPackage969:', dataPackage)

        // average and dev
        const scatter_data = dataPackage.transactionTsne.map(item => item.yld_to_mrty);
        ...
        store.dispatch('lassoInteraction/updateAllData', payload)
          // .then(() => {
          //   // 在 action 完成后执行的代码
          //   console.log('updateAllData 完成', clonedDataPackage.transaction)
          // });
