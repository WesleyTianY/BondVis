<template>
  <div>
    <b-card no-body style="height:100%;">
      ...
      <b-card-body style="overflow: scroll; height: 570px; padding:2px">
        <div style="display: flex; flex-direction: column;">
          <!-- Search Module -->
          <div style="flex: 1;">
            <div style="display:block; height:20px; margin-top: 0px; float: right;">
              <b-input-group size="sm" style="width:278px">
                <b-form-input v-model="searchQuery" placeholder="Search by Name or ID"></b-form-input>
                <template #append>
                  <b-input-group-text 
...
import { fetchBondSummaryData, fetchBondSummaryData_ByBond_cd } from '../../../api/bond.js';
...
export default {
  ...
  methods: {
    onSearchClick() {
      console.log(this.searchQuery); // 输出 searchQuery 内容

      // 初始化 bond_cd_list 为数组（如果不在 data 中初始化）
      if (!this.bond_cd_list) {
        this.bond_cd_list = [];
      }

      // 检查 bond_cd_list 中是否已有 searchQuery，避免重复添加
      if (!this.bond_cd_list.includes(this.searchQuery)) {
        this.bond_cd_list.push(this.searchQuery);
        this.fetchDataByBond_cd(this.queryDate, this.searchQuery);
      } else {
        console.log(`Bond code ${this.searchQuery} is already in the list.`);
      }
    },
    async fetchDataByBond_cd(queryDate, Bond_cd) {
      try {
        await setQueryDate(queryDate)
        // 调用 fetchBondSummaryData 并传入 Bond_cd 参数
        const data = await fetchBondSummaryData_ByBond_cd(Bond_cd); // 假设传入一个包含 Bond_cd 的数组
        // 将返回的数据保存到 bond_list
        // this.bond_list.push(data);
        // 触发 data-loaded 事件，并传递 bond_list 数据
        this.$emit('data-loaded', this.bond_list);
        console.log('Data loaded for Bond_cd, new data:', this.bond_list, data);
        const combinedArray = [...this.bond_list, ...data]
        console.log('combinedArray:', combinedArray);
        this.bond_list = combinedArray
        this.marcoDataLoaded = false;
        this.marcoDataLoaded = true;
      } catch (error) {
        console.error('Error fetching data for Bond_cd:', error);
      } finally {
    ...
    async fetchData(queryDate) {
      try {
        ...
        await setQueryDate(queryDate)
        console.log('fetchBondSummaryData data:', queryDate)
        const data = await fetchBondSummaryData(queryDate);
        
        this.bond_list = data;
        this.$emit('data-loaded', this.bond_list);
        console.log('data-loaded', this.bond_list)
        this.marcoDataLoaded = true;
        console.log('marcoData data:', data)
      } catch (error) {
    ...
    timeClick(date) {
        ...
        const queryDate = localDate.toISOString().slice(0, 10)
  ...
  computed: {
    ...
    filteredAndSortedProjects() {
      ...
      console.log("filteredAndSortedProjects this.bond_list", this.bond_list)
