...
export default {
  ...
  methods: {
    ...
    handleChangeBondSelection(bond_cd) {
      if (this.bond_cd === bond_cd) {
        this.selected = !this.selected;
      }
    }
  },
  created() {
    // this.$root.$on('ChangeBondSelection', this.handleChangeBondSelection);
    this.$root.$on('ChangeBondSelectionClose', (bond_cd) => {
      if (bond_cd == this.bond_cd) {
        this.selected = !this.selected
      }
    })
  },
  beforeDestroy() {
    // this.$root.$off('ChangeBondSelection', this.handleChangeBondSelection);
  }
}
</script>
