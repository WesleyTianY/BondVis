      ...
      class MultiChart {
        ...
        drawValuationLine(data, xOffset, time_Period, startTimePoint, endTimePoint, groupCount) {
          function weightedAverage(data) {
               ...
               * 计算给定数据数组的加权平均值。
               * 
               * @param {Array} data - 包含每个交易项的对象数组，每个对象中包含 'transactionVolume' 和 'netPrice' 属性。
               * @returns {number} - 加权平均值。
               */
              const totalVolume = data.reduce((sum, item) => sum + item.transactionVolume, 0);
              const weightedSum = data.reduce((sum, item) => sum + (item.transactionVolume * item.netPrice), 0);
          ...
          const area = d3.area()
            ...
            .y0(d => yScale(weightedAverage(data)))  // Lower bound
            .y1(d => yScale(weightedAverage(data)+0.2));  // Upper bound
        ...
        drawMktPriceLine(data, xOffset, time_Period, startTimePoint, endTimePoint, groupCount) {
          ...
          const xScale = d3.scaleTime().range([0, this.theWidth])
            // .domain([new Date(startTimePoint), new Date(endTimePoint)])
            .domain([d3.min(data, d => d.timeStamp), d3.max(data, d => d.timeStamp)])
          console.log("this.lower, this.upper 2", this.lower, this.upper)
          const yScale = d3.scaleLinear().range([maxY, minY])
            .domain([this.lower, this.upper])
            // .domain([2.23, 2.32])
          // const yScale = d3.scaleLinear().range([0, this.height * 0.8])
          //   .domain([this.lower, this.upper])
            // .domain([d3.min(data, d => d.netValue), d3.max(data, d => d.netValue)])
          const line = d3.line().x(d => xScale(d.timeStamp)).y(yScale(100.7))
          console.log('line 290:', data)
          const area = d3.area()
            .x(d => xScale(d.timeStamp))
            .y0(yScale(100.7-0.4))  // Lower bound
            .y1(yScale(100.7+0.4));  // Upper bound
          this.lineContainer.append('g').append('path')
            .datum(data)
            .attr('class', 'line')
            .attr('d', line)
        ...
        drawTrasactionScatter(data, xOffset, time_Period, startTimePoint, endTimePoint, groupCount) {
          ...
          //       .text(" σ "))
          // 为图表容器添加鼠标滚轮事件监听器
          const fixedYValue = this.allData.valuation[0].netPrice; // 假设你想要的固定纵轴值为 50，可以根据实际情况修改
          const line_val = d3.line()
          .x(d => xScale(d.timeStamp))
          .y(() => yScale(fixedYValue));


          const lineContainer = this.lassoContainer.append('g')
          const areaContainer = this.lassoContainer.append('g')
          ...
          lineContainer.append('g').append('path')
            ...
            .style('stroke-width', 2)
            .style('fill', 'none')
          const xScaleLinear = d3.scaleLinear().range([0, (this.theWidth)])
            .domain([d3.min(data, d => d.timeStamp), d3.max(data, d => d.timeStamp)])
          const area_val = d3.area()
            .x(d => xScaleLinear(d.timeStamp))
            .y0(d => yScale(fixedYValue-0.2))  // Lower bound
            .y1(d => yScale(fixedYValue+0.2));  // Upper bound
          // areaContainer.append('g').append("path")
          //   .datum(data)
