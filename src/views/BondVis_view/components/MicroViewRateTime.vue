  ...
  methods: {
    ...
    async drawMultiCharts(data, bond_cd, endDay, duration_days, theLimits, colorMapping) {
      ...
      class MultiChart {
        ...
        async drawMultiDayData(dimension) {
          ...
          //     // 直接使用原始数据进行绘图，不进行后续的分组和处理
          //     // 假设这里有一个 drawSingleLineData 函数来绘制单条线
          //     // this.drawSingleLineData(singleFeaList, dimension);
          //     const globalOffset = 100
          //     this.width = this.width - globalOffset
          //     const morningWidth = 2 * this.width / groupCount  * 3 / 7
          //     const xOffset_morning = 2 * 1 / groupCount * this.width + globalOffset  /* 计算偏移 */
          //     const xOffset_afternoon = xOffset_morning + morningWidth
          //     this.drawDayData(singleFeaList, dimension, '2024-07-05', xOffset_morning, 'AM', groupCount)
          //     this.drawDayData(singleFeaList, dimension, '2024-07-05', xOffset_afternoon, 'PM', groupCount)
          //     return;
          // }
          const groupedData = {}
        ...
        drawDayData(data, dimension, date, xOffset, time_Period, groupCount) {
          ...
          switch (dimension) {
            case 'mktPrice':
              ...
              break
            case 'valuation':
              this.drawValuationLine(data, xOffset, time_Period, startTimePoint, endTimePoint, groupCount)
              break
            case 'volume':
        ...
        drawMktPriceLine(data, xOffset, time_Period, startTimePoint, endTimePoint, groupCount) {
          ...
          const line = d3.line().x(d => xScale(d.timeStamp)).y(d => yScale(d.netValue))
          const area = d3.area()
        ...
        drawValuationLine(data, xOffset, time_Period, startTimePoint, endTimePoint, groupCount) {
          // var parseTime = d3.timeParse('%Y/%m/%d %H:%M:%S')
          const mktData = this.allData.mktPrice
          mktData.forEach(function(d) { 
            d.timeStamp = new Date(d.timeStamp)
          })
          mktData.sort((a, b) => a.timeStamp - b.timeStamp)
          startTimePoint = this.formatDateToCustomDateString(startTimePoint)
          endTimePoint = this.formatDateToCustomDateString(endTimePoint)
          const morningWidth = 2 * this.width / groupCount * 3 / 7
          const afternoonWidth = 2 * this.width / groupCount * 4 / 7
          ...
          const maxY = 0; // 最大值
          const xScale = d3.scaleTime().range([0, this.theWidth])
            .domain([d3.min(mktData, d => d.timeStamp), d3.max(mktData, d => d.timeStamp)])
          const yScale = d3.scaleLinear().range([maxY, minY])
            .domain([this.lower, this.upper])
          // const line = d3.line().x(d => xScale(d.timeStamp)).y(yScale(data.yld_to_mrty))
          const fixedYValue = data.yld_to_mrty; // 假设你想要的固定纵轴值为 50，可以根据实际情况修改
        ...
        // drawValuationLine1(data, xOffset, startTimePeriod, endTimePeriod) {
      ...
      console.log('multiChartInstance:', data)
      ...
      // await multiChartInstance.drawMultiDayData('valuation')
    ...
    async fetchData() {
      ...
      try {
        ...
        // average and dev
        const scatter_data = dataPackage.transaction.map(item => item.yld_to_mrty);
